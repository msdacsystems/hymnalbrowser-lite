...
class Software {
  ...
  static VERSION_TAG := Format("v{1}{2}",
    this.VERSION, (this.VERSION_LABEL != "Release" ? (StrLen(this.VERSION_LABEL) > 0 ? '-' StrLower(this.VERSION_LABEL) : '') : ''))
  ...
  static MANUAL_DOWNLOAD_URL_LATEST := "https://knv.li/hbl-latest"
  ...
  static DIRS_HYMNAL_PACKAGE := [
    A_ScriptDir,
    this.DIR_PROGRAM,
    this.DIR_DOCS_PROGRAM
  ]    ;; Scope of directories to be scanned for HymnalDB; In order of priority
  ...
  static GLB_FONT_SIZE := 10                                                               ;; Global font size
  static GLB_FONT_NAME := "Segoe UI Variable"                                                      ;; Global font name
  ...
  static GenerateMetadata() {
    ...
    */
    meta := Object()
    meta.DefineProp("SOFTWARE", {
      value: Software.FULLNAME_VERSION_LABEL
    })
    meta.DefineProp("AUTHORS", {
      value: Software.AUTHORS
    })
    meta.DefineProp("DEV_MODE", {
