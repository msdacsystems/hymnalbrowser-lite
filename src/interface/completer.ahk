...
class UICompleter {
  ...
  __New() {
    ...
    this.DATA := Object()
    this.DATA.DefineProp("LIT", {
      value: []
    })                                            ;; Array of the items in completer (List Items / LIT)
    this.DATA.DefineProp("IDX", {
      value: 0
    })                                             ;; Cached index number of completer
    this.DATA.DefineProp("FCS", {
      value: ''
    })                                            ;; Cached current item text of completer
    this.ACTIVE := false
    ...
    */
    if this.ACTIVE {                                                                    ;; Only record an index and text value if the completer is present & active
      try {
        ...
        Console.Error("UICompleter: " e.Message)
      }
    }
  ...
  KeyPress(key, hk) {
    ...
    */
    ...
    if ArrayMatch(key, [
      ...
      'WheelUp',
      'WheelDown'
    ]) {              ;; Handles behavior of Previous and Next item in completer (ListBox)
      if ArrayMatch(key, [
        'WheelUp',
        'WheelDown'
      ]) {
      ...
      try {
        ...
        Console.Error('UICompleter: Unable to process item actions. ' e.Message)
        Console.Error("UICompleter: Source trigger: " key)
        Console.Error("UICompleter: Current List Size: " SES.SUGGESTIONS)
        Console.Error("UICompleter: Current Index: " this.DATA.IDX)
        Console.Error("UICompleter: Current Focused Item: " this.DATA.FCS)
        Console.Error("UICompleter: Matched Items: " ArrayAsStr(this.DATA.LIT))
      }
    }
    ...
    if ArrayMatch(key, [
      'LButton Up',
      'LButton Down'
    ]) {                                ;; Simulates an action when clicking on an item.
  ...
  GetSpawnPoint(&X, &Y) {
    ...
    */
    ...
    Y += 86                                                                             ;; Offset from the search bar
  }
