...
class HymnalDB {
   ...
   * @returns {Object} An object with properties CAT, NUM, TTL, and EXT.
   */
  static SplitHymn(file) {
    OUT := Object()
    OUT.DefineProp("CAT", {
      value: SubStr(file, 1, 2)
    })
    OUT.DefineProp("NUM", {
      value: (isDigit(SubStr(file, 4, 3)) ? SubStr(file, 4, 3) : 0)
    })
    OUT.DefineProp("TTL", {
      value: SubStr(file, 8, -5)
    })
    OUT.DefineProp("EXT", {
      value: (StrSplit(file, ".").length > 1 ? StrReplace(StrSplit(file, ".")[-1], "`r") : "")
    })
    return OUT
  }
  ...
  static _VerifyDatabase() {
    ...
    for dir in SW.DIRS_HYMNAL_PACKAGE {
      Console.Verbose("HymnsDB: Scanning from directory `"" dir '"')
      if FileExist(PathJoin(dir, CF.HYMNAL.PACKAGE)) {
        Console.Verbose("HymnsDB: Database was found.")
        CF.__FILE_HYMNALDB := PathJoin(dir, CF.HYMNAL.PACKAGE)
        break
      }
      Console.Verbose("HymnsDB: Database was not found.")
    }
  ...
  static ScanHymnal() {
    ...
    Console.Info("HymnsDB: Scanning hymnal from `"" CF.__FILE_HYMNALDB '"')
    ...
    if INVALID_ITEMS.Length {
      Console.Warn(Format("HymnsDB: Detected {1} invalid item(s)", INVALID_ITEMS.Length))
    }
    Console.Info(
