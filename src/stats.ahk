/**
 * 
 * Hymnal Browser Lite - Statistical Data Management
 * -------------------------------------------------
...
ISO_STRING := "yyyy-MM-ddTHH:mm:ssZ"
...
class Stats {
    ...
    Load() {
        ...
        if !FileExist(this.FILE) {
            _LOG.Error("Stats: File does not exist")
            return
        }

        _LOG.Info("Stats: Loading stats from JSON file: " . this.FILE)

        file := FileOpen(this.FILE, "r")
        if !file {
            _LOG.Error("Stats: Failed to stats file for reading")
            return
        }
        ...
        if !parsed {
            _LOG.Error("Stats: Stats file is empty")
            return
        }
        ...
        for i in parsed {
            statObj := MapToObj(i)
            if !statObj.id {
                _LOG.Error("Stats: Invalid JSON file")
                return
            }
            this.HymnData.DefineProp(statObj.id, { value: Stat(statObj.id) })
        ...
        _LOG.Info(Format("Stats: Loaded {1} hymn stats", parsed.length))
    }
    ...
    GenerateStat(Hymn) {
        if (this.HasStat(Hymn)) {
            _LOG.Error("Stats: Hymn stat already exists")
    ...
    Export() {
        ...
        if !file {
            _LOG.Error("Stats: Failed to open file for writing")
            return
        }

        file.Write(data)
        file.Close()
        _LOG.Verbose("Stats: Exported stats to JSON file")
    }
...
class Stat {
    ...
    IncrementLaunch() {
        this.launches++
        this.launchedAt := FormatTime(, ISO_STRING)
    }
    ...
    IncrementQuery() {
        this.queries++
        this.queriedAt := FormatTime(, ISO_STRING)
    }
