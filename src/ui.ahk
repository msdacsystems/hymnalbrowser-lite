...
class UI {
  ...
  static Setup() {
    ...
    for classObj in UI.UIs {
      UI.DefineProp(StrUpper(classObj._NAME), {
        value: classObj.Call()
      })               ;; Initiate GUI Setup for the class; Store the class instance
    }
    System.FixCtrlBackspace(UI.MAIN.GUI.Hwnd)
    UI.ConnectEvents()
    UI.StartThreads()
    UI.Keybinds()
    Console.Info(Format("UI: Setup finished ({1} ms)", A_TickCount - ST))
  }
  ...
  static ConnectEvents() {
    Console.Verbose("UI: Connecting UI Events")
  ...
  static StartThreads() {
    Console.Verbose("UI: Starting UI Listener Threads")
  ...
  static Keybinds() {
    Console.Verbose("UI: Binding keys")
  ...
  static LeftMousePressEvent(args*) {
    ...
    if WinExist(System.AHK_ID) == W {
      if !UI.ACTIVE {
        Console.Verbose('UI: Window is activated from MouseClick')
        UI.SetActive()
      }
  ...
  static OpenWebsite(args*) {
    try {
      ...
      Console.Error("UI: Unable to open developer website")
    }
  }
